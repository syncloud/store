    ...
    [
        ...
        },
        {
            ...
            command: [|||
                ...
                GARAGE_PID=$!
                for i in $(seq 120); do
                    /garage status 2>/dev/null | grep -qE "(NO ROLE|HEALTHY)" && break
                    sleep 1
                done
                ...
                /garage bucket allow --read --write --owner test --key test || echo "allow failed"
                /garage bucket website --allow test          || echo "website failed"
                echo "garage init complete; status:"
                /garage status
                wait $GARAGE_PID
